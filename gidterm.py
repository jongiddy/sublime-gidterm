...
class DisplayPanel(GidTermPanel):
    ...
    def set_tab_label(self, command, extra=''):
        # type: (list[str], str) -> None
        if extra:
            size = TITLE_LENGTH - len(extra) - 1
            name = '{}\ufe19{}'.format(self.make_label(size, command), extra)
        else:
            name = self.make_label(TITLE_LENGTH, command)
...
class LivePanel(GidTermPanel):
    ...
    def set_title(self, extra=''):
        self.display_panel.set_tab_label(self.command, extra)
